...
features = np.load(path + r"/Interim/features.npy")
labels = np.load(path + r"/Interim/labels.npy")

# Optionally: Split the data into training and testing sets
X_train, X_test, y_train, y_test = train_test_split(features, labels, test_size=0.2, random_state=29)
...
X_train_sub, X_val, y_train_sub, y_val = train_test_split(X_train, y_train, test_size=0.1, random_state=31)
...
bst.save_model(path + "/Models/xgboost_model_random.json")
